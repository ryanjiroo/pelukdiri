...
const Register = () => {
    ...
    const { register, isLoading, currentUserToken } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        if (password !== confirmPassword) {
            // Notifikasi ini akan ditimpa oleh modal custom dari AuthContext
            // Jadi tidak perlu alert() di sini, cukup return.
            return;
        }
        // Asumsi 'email' digunakan sebagai 'username' untuk API register
        const success = await register(email, password, phone); // Pass phone to register
        if (success) {
    ...
    return (
        <div className="min-h-screen mt-16 bg-[#092635] flex items-center justify-center p-4">
            <div className="bg-[#1B4242] rounded-2xl shadow-xl p-8 md:p-10 w-full max-w-md">
                ...
                <div className="flex justify-center mb-6">
                    <span className="text-4xl font-extrabold text-[#9EC8B9] flex items-center gap-2">
                        <svg
                            ...
                            className="h-10 w-10 text-[#9EC8B9]"
                            viewBox="0 0 24 24"
                            fill="none"
                            // Perbaiki atribut SVG dari kebab-case ke camelCase
                            stroke="currentColor"
                            strokeWidth="2"
                            strokeLinecap="round"
                ...
                <p className="text-[#D6FFEF] text-center mb-8">
                    Buat akun PelukDiri Anda sekarang.
                </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <label htmlFor="phone" className="block text-sm font-medium text-[#9EC8B9] mb-2">
                            Nomor Telepon
                        </label>
                        <div className="relative">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <svg className="h-5 w-5 text-[#5C8374]" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6.62 10.79a15.053 15.053 0 006.59 6.59l2.2-2.2a1 1 0 011.04-.26 11.23 11.23 0 004.29 1.15 1 1 0 011 .99V20a1 1 0 01-1 1A17 17 0 013 4a1 1 0 011-1h3.5a1 1 0 01.95.78 11.23 11.23 0 001.15 4.29 1 1 0 01-.26 1.04l-2.2 2.2z"/>
                                </svg>
                            </div>
                            <input
                                id="phone"
                    ...
                    <div>
                        <label htmlFor="confirmPassword" className="block text-sm font-medium text-[#9EC8B9] mb-2">
                ...
                <p className="mt-4 text-sm text-[#D6FFEF] text-center">
                    Sudah punya akun?{' '}
                    <Link to="/login" className="font-medium text-[#5C8374] hover:text-[#9EC8B9]">
                        Masuk di sini
                    </Link>
                </p>
            </div>
